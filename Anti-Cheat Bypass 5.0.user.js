...
(function () {
    ...
    const generateWithOpenRouter = async (prompt) => {
        ...
        for (let i = 0; i <= 2; i++) {
            if (i > 0) {
                // lastResult.response may be undefined on network-error path (status 0)
            ...
            // Only retry on transient errors: rate-limit (429), service-unavailable (503),
            // or network-error (status 0)
            const isTransient = lastResult.status === 429
                || lastResult.status === 503
                || lastResult.status === 0;
            if (!isTransient) break;
        }

        // Primary model exhausted — try fallback free models when rate-limited or unavailable
        const rateLimited = lastResult.status === 429
            || lastResult.status === 503
            || lastResult.status === 0
            || /rate.limit|provider.returned.error/i.test(lastResult.errMsg || '');

        // Track the last fallback error separately so the final throw is accurate
        let lastFallbackResult = null;

        if (rateLimited) {
            for (const fb of FREE_FALLBACKS) {
                console.log(`[OpenRouter] Primary exhausted. Trying fallback: "${fb}"`);
                ...
                if (fbTransient) {
                    ...
                    console.log(`[OpenRouter] Fallback "${fb}" rate-limited/overloaded, waiting ${waitMs / 1000}s...`);
